...
// The function generates a random 10-digit account number,
// checks if it already exists in the database, and generates a new one until it is unique.
exports.createAccount = (req, res) => {
  let flag = true;
  let accountNumber = Math.floor(1000000000 + Math.random() * 9000000000);
  Account.findOne({ accountNumber: accountNumber }).exec(
    async (error, account) => {
      if (account) {
      ...
      // It then retrieves the account type and initial balance from the request body.
      //Further creates a new account in the database using the Mongoose library.
...
// This function looks up an Account document in the database with a matching accountNumber field value in the request body. 
// If found, the accountStatus of the document is updated to the accountStatus field value in the request body and saved to the database.
exports.activateAccount = (req, res) => {
  Account.findOne({ accountNumber: req.body.accountNumber }).exec(
    async (error, account) => {
...
exports.getUserAccounts = (req, res) => {
  ...
  Account.find({ customerID: userId }).exec(async (error, accounts) => {
    if (error) return res.status(400).json({ error });
    let totalBalance = 0;
