...
exports.signin = (req, res) => {
  User.findOne({ email: req.body.email }).exec(async (error, user) => {
    ...
    if (user) {
      ...
      if (
        user.authenticate(req.body.hash_password) == "true" &&
        user.role === "admin"
      ) {
...
exports.isAuthorized = (req, res, next) => {
  User.findOne({ email: req.body.email }).exec(async (error, user) => {
    ...
    if (error) return res.status(400).json({ error });
    User.updateOne({ email: email }, { $set: { authorized: authorized } }).exec(
      (error, user) => {
        if (error) return res.status(400).json({ error });
        res.status(200).json({
...
exports.getAccounts = (req, res) => {
  ...
  Account.find({ customerID: req.params.id }).exec(async (error, accounts) => {
    console.log(req.params.id);
    if (error) return res.status(400).json({ error });
    res.status(200).json({
      accounts,
...
exports.allAccounts = (req, res) => {
  Account.find({}).exec(async (error, accounts) => {
    if (error) return res.status(400).json({ error });
